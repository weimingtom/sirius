...
{ 
     ...
     *  
     * @access public
	 * @param int $since_id 若指定此参数，则只返回ID比since_id大的微博消息
    ...
    function home_timeline($since_id = null, $page = 1, $count = 20, $max_id = null) 
    {
		...
		return $this->oauth->get("http://api.t.sohu.com/statuses/friends_timeline.json" , $param ); 
    } 
    ...
    { 
		...
		if( $page ) $param['page'] = $page; 
   		if( $count ) $param['count'] = $count; 			
    	if( $max_id ) $param['max_id'] = $max_id; 
		return $this->oauth->get("http://api.t.sohu.com/statuses/mentions.json" , $param ); 
    } 
    ...
    {
    	if ($uid_or_name !== null) {
    		return $this->oauth->get('http://api.t.sohu.com/users/show/' . urlencode($uid_or_name) . '.json');
    	} else {
        	return $this->oauth->get('http://api.t.sohu.com/users/show.json');
    	} 
    } 
	...
	function user_timeline($since_id = null, $page = 1 , $count = 20, $max_id = null, $uid_or_name = null) 
    {
		...
		return $this->oauth->get("http://api.t.sohu.com/statuses/user_timeline/" . $uid_or_name . ".json" , $params ); 
    } 
    ...
    { 
		...
		if( $page ) $param['page'] = $page; 
   		if( $count ) $param['count'] = $count; 			
    	if( $max_id ) $param['max_id'] = $max_id; 
		return $this->oauth->get("http://api.t.sohu.com/direct_messages.json" , $params ); 
    } 
    ...
    { 
        ...
        return $this->oauth->get('http://api.t.sohu.com/statuses/comments/' .  $sid . '.json', $param ); 
    ...
    { 
        ...
        return $this->oauth->get('http://api.t.sohu.com/statuses/repost_timeline.json' , $param ); 
	...
	function trend_timeline($trand_name) {
		return $this->oauth->get( 'http://api.t.sohu.com/statuses/search.json', array('q'=> $trand_name)); 
	}
     ...
     * @ignore 
     */ 
    protected function request_with_pager_and_since_id( $url , $page = false , $count = false, $since_id = false ) 
    { 
        $param = array(); 
        if( $page ) $param['page'] = $page; 
        if( $count ) $param['count'] = $count;
     ...
     * @ignore 
     */ 
    protected function request_with_uid_and_since_id( $url , $uid_or_name , $page = false , $count = false, $since_id = false, $cursor = false , $post = false ) 
    { 
        $param = array(); 
        if( $page ) $param['page'] = $page; 
        if( $count ) $param['count'] = $count; 
