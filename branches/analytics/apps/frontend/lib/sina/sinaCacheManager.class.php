...
class sinaCacheManager {
	...
	private function __construct() {
		...
		$this->countPerCall = intval(sfConfig::get('app_sina_count_per_call', 200));
		$this->cacheFreshTheshold = intval(sfConfig::get('app_sina_cache_refresh_threshold', 50));
	...
	public function home_timeline($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'sina_' . 'list_home_' . $profile_id;
	...
	public function list_dm($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'sina_' . 'list_dm_' . $profile_id;
	...
	public function mentions($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'sina_' . 'list_mentions_' . $profile_id;
	...
	public function cacheMessages($data) {
		if (isset($data['id'])) {
			...
			foreach ($data as $message) {
				$this->cache->set('sina_message_' . $message['id'], $message);
			}
		}
	}
	...
	protected function getMessagesFromCache($idArray) {
		...
		foreach ($idArray as $id) {
			$result[] = $this->cache->get('sina_message_' . $id);
		}
		return $result;
	}
}
