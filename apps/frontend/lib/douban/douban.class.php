...
{ 
     ...
     * @return array 
     */ 
    function home_timeline($count = NULL, $start = NULL) 
    {
    	$params = array();
    	if ($count != NULL) $params['max-results'] = $count;
		if ($start != NULL) $params['start-index'] = $start;
        return $this->oauth->get('http://api.douban.com/people/%40me/miniblog/contacts', $params); 
    } 
    /** 
     * 发表微博 
     *  
    ...
    {
    	if ($name == null) 
        	return $this->oauth->get( 'http://api.douban.com/people/%40me');
		else
			return $this->oauth->get( 'http://api.douban.com/people/' . $name);
    } 
     ...
     * @return array 
     */ 
    function user_timeline($count = NULL, $start = NULL, $username = NULL) 
    {
    	$params = array();
    	if ($count != NULL) $params['max-results'] = $count;
		if ($start != NULL) $params['start-index'] = $start;
		...
		if ($username != NULL) {
			$data = $this->oauth->get('http://api.douban.com/people/' . $username . '/miniblog', $params);
		} else {
			$data = $this->oauth->get('http://api.douban.com/people/%40me/miniblog', $params);
		}
		
		for ($i = count($data['entry'])-1; $i >= 0; $i--) {
			$data['entry'][$i]['author'] = $data['author'];
		}
		return $data;
    } 

    function list_dm($count = NULL, $start = NULL) 
    {
    	$params = array();
    	if ($count != NULL) $params['max-results'] = $count;
		if ($start != NULL) $params['start-index'] = $start;
		return $this->oauth->get( 'http://api.douban.com/doumail/inbox', $params); 
    } 
...
class doubanOAuth { 
     ...
     * @ignore 
     */ 
    function accessTokenURL()  { return 'http://www.douban.com/service/auth/access_token'; } 
    /** 
     * @ignore 
     */ 
     ...
     * @ignore 
     */ 
    function authorizeURL()    { return 'http://www.douban.com/service/auth/authorize'; } 
    /** 
     * @ignore 
     */ 
    function requestTokenURL() { return 'http://www.douban.com/service/auth/request_token'; } 
    ...
    function get($url, $parameters = array()) { 
    	if ($this->format === 'json' && !isset($parameters['rel'])) {
    		$parameters['alt'] = 'json';
    	}
