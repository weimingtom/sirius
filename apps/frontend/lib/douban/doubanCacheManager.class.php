...
class doubanCacheManager {
	...
	private function __construct() {
		...
		$this->countPerCall = intval(sfConfig::get('app_douban_count_per_call', 20));
		$this->cacheFreshTheshold = intval(sfConfig::get('app_douban_cache_refresh_threshold', 50));
	}
	...
	public function home_timeline($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'douban_' . 'list_home_' . $profile_id;
	...
	public function list_dm($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'douban_' . 'list_dm_' . $profile_id;
	...
	public function trend_timeline($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20, $trend_name = NULL) {
		...
		$cacheMessageListName = 'douban_' . 'trend_' . $trend_name . '_' .$profile_id;
	...
	public function mentions($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20) {
		$cacheMessageListName = 'douban_' . 'list_mentions_' . $profile_id;
	...
	public function user_timeline($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20, $uid_or_name = null) {
		$cacheMessageListName = 'douban_' . 'list_user_' . $uid_or_name . '_' . $profile_id;
	...
	public function favorite_timeline($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20, $uid_or_name = null) {
		$cacheMessageListName = 'douban_' . 'list_favorite_' . $profile_id;
	...
	public function get_reposts_by_sid($profile_id, $apiConsumer, $since_id = null, $before_id = null, $count = 20, $sid = null) {
		...
		$cacheMessageListName = 'douban_' . 'list_retweet_' . $sid . '_' . $profile_id;
...
//$count =1;
		...
		if ($before_id === null) {
			if (time() - $cacheMessageListLastModify > $this->cacheFreshTheshold) {
				$data = $apiConsumer->$functionName($this->countPerCall, null, $addtional_arg1, $addtional_arg2, $addtional_arg3);
				if ($data['ret'] != 0) {
					if (sfConfig::get('sf_logging_enabled'))
					{
						sfContext::getInstance()->getLogger()->error("douban API call incorrect - " . $functionName);
					}
					return array();
				}
				$data = $data['entry'];
			...
			if (array_search($before_id, $cacheMessageList) === FALSE) {
				...
				if ($total - $offset < $count) {
					$data = $apiConsumer->$functionName($this->countPerCall, count($cacheMessageList) - 1, $addtional_arg1, $addtional_arg2, $addtional_arg3);
					if ($data['ret'] != 0) {
						if (sfConfig::get('sf_logging_enabled'))
						{
							sfContext::getInstance()->getLogger()->error("Douban API call incorrect - " . $functionName);
	...
	public function cacheMessages($data) {
		if (isset($data['id'])) {
			$this->cache->set('douban_message_' . end(explode('/', $data['id']['$t'])), $data);
		} else {
			foreach ($data as $message) {
				$this->cache->set('douban_message_' . end(explode('/', $message['id']['$t'])), $message);
			}
		}
	}
	...
	protected function getMessagesFromCache($idArray) {
		...
		foreach ($idArray as $id) {
			$result[] = $this->cache->get('douban_message_' . $id);
		}
		return $result;
	}
}
